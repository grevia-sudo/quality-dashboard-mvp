...
| Google Sheet 同步 | 採購單同步 helper 與同步腳本已擴充到 **T 欄**，回寫欄位名稱為「鏡頭狀態」，未勾選時預設寫入「正常」。 |
...
| `scripts/purchase-sheet-sync-helpers.mjs` | 採購單表頭擴充為 T 欄，增加鏡頭狀態輸出。 |
| `scripts/sync-purchase-sheet.mjs` | Google Sheet 範圍由 A:S 擴充為 A:T，並加入 `cCameraSummary` 查詢與回寫。 |

接下來建議你手動驗證三件事。第一，是在管理後台新增或啟用一個 C 站鏡頭狀態項目後，重新整理 C 站頁面，確認該選項會出現。第二，是在 C 站勾選一個以上鏡頭狀態後完成作業，確認流程能正常推進。第三，是到 Google Sheet 的採購單工作表查看對應列的 **T 欄**，確認未勾選時為「正常」，有勾選時會寫成像「破裂, 刮傷」這樣的摘要。
