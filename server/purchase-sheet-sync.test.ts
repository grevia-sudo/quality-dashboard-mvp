...
describe("purchase sheet sync helpers", () => {
  it("builds a sheet row with normalized blank-safe values", () => {
    ...
    ).toEqual(["PO-1", "綠途未來", "智慧型手機", "", "SN-1", "", "", "2026/04/22 18:30", "2026/04/22 19:43", "2026/04/22 20:08", "88, 電池異常", "正常"]);
  });
  ...
  it("formats Date objects for the sheet using YYYY/MM/DD HH:mm", () => {
    ...
    ).toEqual(["PO-2", "綠途未來", "平板", "BATCH-2", "SN-2", "IMEI-2", "iPad mini", "2026/04/22 09:05", "2026/04/22 10:43", "2026/04/22 11:15", "正常", "後標準相機故障, 鏡頭馬達故障/抖動"]);
  });
  ...
  it("fills only blank cells and preserves non-empty sheet values except H 到 L 欄由系統刷新", () => {
    const existingRow = ["PO-1", "綠途未來", "智慧型手機", "", "SN-1", "IMEI-1", "現場已填品名", "Wed Apr 22 2026 13:47:47 GMT-0400 (Eastern Daylight Time)", "Wed Apr 22 2026 14:43:47 GMT-0400 (Eastern Daylight Time)", "舊的 B 時間", "舊的電池資訊", "舊的故障資訊"];
    const generatedRow = ["PO-1", "綠途未來", "智慧型手機", "BATCH-1", "SN-1", "IMEI-1", "iPhone 13", "2026/04/22 18:30", "2026/04/22 19:43", "2026/04/22 20:08", "88, 電池異常", "正常"];
  ...
  it("creates the standard header row when the sheet is still empty", () => {
    expect(PURCHASE_SHEET_HEADER).toEqual(["採購單號", "廠商", "商品分類", "商品批號", "商品序號", "IMEI", "品名", "點到貨時間", "安裝完成時間", "B站完成時間", "電池檢測", "B站故障狀態"]);
