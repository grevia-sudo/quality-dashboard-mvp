...
import { Activity, ArrowRight, Boxes, ClipboardCheck, Gauge, PackageSearch, ShieldCheck, Sparkles } from "lucide-react";
...
export default function Home() {
  ...
  const dashboardQuery = trpc.dashboard.home.useQuery(undefined, { enabled: isAuthenticated });
      ...
      <div className="space-y-6">
        <section className="grid gap-6 lg:grid-cols-[1.6fr_1fr]">
          <Card className="overflow-hidden rounded-[28px] border-0 bg-[#eef2f7] shadow-sm">
            <CardContent className="relative p-8">
              ...
              <Badge className="bg-white/80 text-slate-700">工程師入口</Badge>
              <h1 className="mt-5 max-w-2xl text-3xl font-black tracking-tight text-slate-900 md:text-4xl">
                依未完成數量快速切換站點，讓現場支援與回補更流暢。
              </h1>
              <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-600 md:text-base">
                每張站點卡片即時顯示未完成數量、今日新增與逾期件數。工程師完成一段作業後可直接返回總覽，再切換到其他站點持續支援。
              </p>
            </CardContent>
          </Card>
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          <Card className="rounded-[26px] border-0 bg-white shadow-sm lg:col-span-2">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base font-bold"><Activity className="h-4 w-4 text-[#7ca3d9]" />MVP 作業節奏</CardTitle>
            </CardHeader>
            <CardContent className="grid gap-4 md:grid-cols-3">
              {[
                { title: '先看總覽', text: '先看各站未完成數量與逾期件數，再決定優先支援哪一站。' },
                { title: '進站掃碼', text: '於站點頁掃碼或輸入商品代碼，立即查看商品資料卡與處理資訊。' },
                { title: '完成後切站', text: '完成站點作業後回到總覽頁，再切往下一站持續支援。' },
              ].map((item) => (
